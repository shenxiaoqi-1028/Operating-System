 ...
 *				allocate_new_mem 				     *
 *===========================================================================*/
PUBLIC int allocate_new_mem(rmp,old_clicks)
register struct mproc *rmp;
phys_clicks old_clicks;
{   
    register struct mem_map *mem_sp, *mem_dp;
    phys_clicks new_clicks, old_base,new_base;
    phys_clicks  old_stack_base,new_stack_base;
   
    
    phys_bytes data_bytes,stak_bytes;
    phys_bytes old_base_bytes,new_base_bytes;
    ...
    old_base_bytes=old_base << CLICK_SHIFT;  
    old_stack_base_bytes=old_stack_base << CLICK_SHIFT;  
    
    new_clicks=2*old_clicks;
    new_base=alloc_mem(new_clicks);
    if(new_base==NO_MEM){
      return(ERROR);
    }
    
    new_base_bytes = (phys_bytes) new_base << CLICK_SHIFT;
    
    if ((x=sys_memset(0,new_base_bytes,(new_clicks<<CLICK_SHIFT)))!=OK){  
	  panic(__FILE__,"new mem can't be zero",x);
    }

    
    new_stack_base=new_base+new_clicks-mem_sp->mem_len;
    new_stack_base_bytes=new_stack_base << CLICK_SHIFT;
    
    
    
    x = sys_abscopy(old_base_bytes,new_base_bytes,data_bytes);
    if (x < 0 ) panic(__FILE__,"allocate_new_mem can't copy",x);
    x = sys_abscopy( old_stack_base_bytes,new_stack_base_bytes,stak_bytes);
    if ( x < 0 ) panic(__FILE__,"allocate_new_mem can't copy",x);
    
   
    rmp->mp_seg[D].mem_phys = new_base;
    rmp->mp_seg[S].mem_phys = new_stack_base;
    rmp->mp_seg[S].mem_vir = mem_dp->mem_vir+new_clicks-mem_sp->mem_len;
...
#define SAFETY_CLICKS ((SAFETY_BYTES + CLICK_SIZE - 1) / CLICK_SIZE)
  ...
  if (lower < gap_base) {/* data and stack collided */
      if (allocate_new_mem(rmp,  
	                 (phys_clicks)(mem_sp->mem_vir+mem_sp->mem_len-mem_dp->mem_vir)) == ERROR)  
	        return(ENOMEM);}  
